...
namespace Awesome.Net.Workflows.Services
{
    public class WorkflowManager : IWorkflowManager
    {
        ...
        private readonly IGuidGenerator _workflowIdGenerator;
        ...
        (
            ...
            IGuidGenerator workflowIdGenerator,
        ...
        public Workflow NewWorkflow(WorkflowType workflowType, string correlationId = null)
        {
            var workflow = new Workflow
            {
                ...
                CreatedUtc = _clock.Now
            };

            workflow.WorkflowId = GuidGeneratorExtensions.Create(_workflowIdGenerator);
            return workflow;
        }
        ...
        public Task<ActivityContext> CreateActivityExecutionContextAsync(ActivityRecord activityRecord, JObject properties)
        {
            ...
            if(activity == null)
            {
                ...
                activity = new MissingActivity(activityRecord);
            }
        ...
        private async Task<object> SerializeAsync(object value)
        {
            ...
            var workflowValueSerializers = _serviceProvider.GetService<IEnumerable<IWorkflowValueSerializer>>();
            await workflowValueSerializers.InvokeAsync(x => x.SerializeValueAsync(context), _logger);
        ...
        private async Task<object> DeserializeAsync(object value)
        {
            var context = new SerializeWorkflowValueContext(value);
            var workflowValueSerializers = _serviceProvider.GetService<IEnumerable<IWorkflowValueSerializer>>();
            await workflowValueSerializers.InvokeAsync(x => x.DeserializeValueAsync(context), _logger);
            return context.Output;
        }
    }
}
